  ...
  async getAllRoutes(
    ...
    for (const itin of sortedItins) {
      for (const leg of itin.legs) {
        ...
        if (pts) {
          globalCoords.push(...polyline.decode(pts).map(([lat, lon]) => ({ lon, lat })));
        }
      }
    }
    ...
    globalCoords.forEach(({ lon, lat }, idx) => {
      ...
      console.log(`[getAllRoutes][ITS] globalCoord[${idx}] lon:${lon}, lat:${lat} -> linkId:${id}`);
      if (id) globalLinkIdSet.add(id);
    });
    console.log('[getAllRoutes][ITS] globalLinkIdSet:', Array.from(globalLinkIdSet));
    // 카테고리별 분류
      ...
      const rawRealtimeArrival: (number | null)[] = await Promise.all(
        transitLegs.map(async (leg: Leg) => {
          ...
          try {
            ...
            const routeId = leg.routeId?.split('TAGO_')[1] ?? leg.routeShortName ?? '';
            const cityCode = await this.tagoService.getCityCodeFromBusStop(
              leg.from.lat.toString(),
              leg.from.lon.toString(),
              nodeId,
            );
            console.log(`[getAllRoutes][BusArrivals] Requesting Tago realtime for nodeId=${nodeId}, routeId=${routeId}`);
            const res = await this.tagoService.getRealtimeBusArrivals(cityCode, nodeId, routeId);
            console.log(`[getAllRoutes][BusArrivals] Raw Tago response for nodeId=${nodeId}, routeId=${routeId}:`, res.data);
      ...
      for (const route of allRoutes[key]) {
        ...
        const stopsCoords = transitSub.flatMap(s => {
          ...
          return inter.length > 0
            ...
            : [{ lon: (s.from as any).lon, lat: (s.from as any).lat }];
        });
        const buffer = 0.001;
        ...
        console.log(`[getAllRoutes][ITS] Registering realtime call for bboxKey=${bboxKey}, params={minX:${minX},maxX:${maxX},minY:${minY},maxY:${maxY}}`);
        (route as any)._bboxKey = bboxKey;
        rtPromises.set(
          bboxKey,
          this.itsService.getRealtimeTrafficInfo({ type: 'all', getType: 'json', minX, maxX, minY, maxY }),
        );
        // its.controller와 동일하게 globalLinkIdSet 사용
        (route as any)._linkIdSet = globalLinkIdSet;
        if ((route as any)._sectionId) fcSectionIds.add((route as any)._sectionId);
        // stopsCoords 매핑 로그
        stopsCoords.forEach(({ lon, lat }, idx) => {
          const sid = this.mapper.findLinkId(lon, lat);
          console.log(`[getAllRoutes][ITS] category=${key}, stopCoord[${idx}] lon:${lon}, lat:${lat} -> linkId:${sid}`);
        });
      }
    }
    ...
    for (const [key, promise] of rtPromises) {
      console.log(`[getAllRoutes][ITS] Awaiting realtime promise for bboxKey=${key}`);
      const rt = await promise;
      console.log(`[getAllRoutes][ITS] Raw ITS response for bboxKey=${key}:`, rt);
      ...
      console.log(`[getAllRoutes][ITS] Parsed ITS items for bboxKey=${key}:`, itemsArr);
      rtResults.set(key, itemsArr);
    }
    // 예측정보 호출 준비 및 실행
    const fCastDate = options?.date ? options.date.replace(/-/g, '') : new Date().toISOString().slice(0, 10).replace(/-/g, '');
    const fCastHour = options?.time ? options.time : new Date().getHours().toString().padStart(2, '0');
    const fcPromises = new Map<string, Promise<any>>();
    for (const sec of fcSectionIds) {
      fcPromises.set(sec, this.itsService.getForecastInfo({ sectionId: sec, fCastDate, fCastHour, getType: 'json' }));
    }
    ...
    for (const key of Object.keys(allRoutes) as (keyof AllRoutesDataDto)[]) {
      for (const route of allRoutes[key]) {
        ...
        if (!linkIdSet) {
          console.log('[getAllRoutes][ITS] No linkIdSet for route, skipping ITS for category', key);
        ...
        console.log('[getAllRoutes][ITS] globalLinkIdSet:', Array.from(linkIdSet));
        const filteredItems = allItems.filter(item => linkIdSet.has(item.linkId));
        console.log('[getAllRoutes][ITS] filteredItems:', filteredItems.map((it: any) => it.linkId));
        if (filteredItems.length > 0) {
          route.main.trafficItems = filteredItems;
        } else if (allItems.length > 0) {
          console.log('[getAllRoutes][ITS] No intersection, using all ITS items');
